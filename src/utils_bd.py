...
class DBManager(abstrate_dbmaneger):
    ...
    def create_database(self) -> None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                ALTER TABLE vacancies ADD CONSTRAINT company_id FOREIGN
                 KEY(company_id) REFERENCES companies(company_id);
            """
            )
    ...
    def insert_data_to_table(
        self, list_company: list[Any] = [], list_vacancy: list[Any] = []
    ) -> None:
        ...
        with conn.cursor() as cur:
            if list_company:
                for item in list_company:
                    cur.execute(
                        f"INSERT INTO companies (company_id, company_name, company_url)"
            ...
            if list_vacancy:
                for item in list_vacancy:
                    print(item)
                    cur.execute(
                        f"INSERT INTO vacancies (vacanscies_id, vacancy_name, company_id,"
                        f" salary_from, salary_to, vacansy_url)"
                        f" VALUES {item[0], item[1], item[2], item[5], item[4], item[6]};"
                    )
    ...
    def get_all_vacancies(self) -> list[Any]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                        SELECT vacancies.vacancy_name, companies.company_name, vacancies.salary_from,
                         vacancies.salary_to, vacancies.vacansy_url
                        FROM companies INNER JOIN vacancies on companies.company_id = vacancies.company_id
                        """
            )
    ...
    def get_vacancies_with_keyword(self, looking_word: str) -> list[Any]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT vacancies.vacancy_name, companies.company_name, vacancies.salary_from, "
                f"vacancies.salary_to, vacancies.vacansy_url "
